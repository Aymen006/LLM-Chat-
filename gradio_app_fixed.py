#!/usr/bin/env python3
"""
Gradio chatbot interface with persistent memory (Supabase) + Auto-Save
...
from db_functions import get_or_create_coach_state, save_coach_state, save_turn_pair, load_recent_turns
...
def process_message(user_message, history, user_id, conv_history, user_msg_count):
    ...
    # We use DB history instead of session-only conv_history
    ...
    # Phase 2: We don't need in-memory history anymore as we load from DB.
    # Return empty list to keep State object valid but empty.
    new_conv_history = []
